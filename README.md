...
- `POST /internal/v1/reconcile` (admin) — reclaim **orphan blobs**: bytes with no `files`/`file_variants` row (e.g. an upload that wrote the blob then crashed before its DB row). Safe to run live (grace window, floored at 60s, + a per-key DB re-check before each delete); supports `dry_run`. **The blob store must be dedicated to ByteHangar** — reconcile treats every unreferenced object as an orphan. Run occasionally and after a restore.
