...
if ! curl -s --retry 40 --retry-connrefused --retry-delay 1 "http://localhost:$PORT/health" >/dev/null; then
  echo "server did not become healthy; log:"; cat "$ROOT/.e2e-server.log"; exit 1
fi
echo "==> server healthy"

echo "==> running e2e"
BYTEHANGAR_URL="http://localhost:$PORT" ADMIN_TOKEN="$ADMIN_TOKEN" node "$ROOT/scripts/e2e.mjs"
